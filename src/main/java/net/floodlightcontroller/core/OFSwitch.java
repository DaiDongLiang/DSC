...
public class OFSwitch implements IOFSwitchBackend {
	 ...
	 *	提供函数查询和更新存储的端口
	 *	这个类确保每个端口名和端口号是独一无二的。
	 *	当更新端口时检查是否因为更新port number <-> port name映射发生改变
	 *	如果一个含有端口号和端口名的新端口与之前的映射不一致，
	 *	将会删除所有跟新端口名和端口号有关的端口，之后添加新端口
